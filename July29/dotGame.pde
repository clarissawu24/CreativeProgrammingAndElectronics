...
boolean gameStart = false;  // game starts false if not in game mode
void keyPressed() {         // game doesn't start until a key is pressed to change value to true 
  gameStart = true;
  score = 0;
}
...
void setup() {
  ...
  in = minim.getLineIn();
  catcher = new Catcher(radius);                 // creating catcher radius 40
...
class Catcher {
  ...
  void display() {                               // create catcher that also grows based on with audio input 
    noStroke();
    fill(c);
    ellipse(x, y, 25+(radius), 25+(radius)); // catcher size changeable using potentiometer 
